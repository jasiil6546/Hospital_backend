...
from .models import Registrationform  # Assuming Registrationform is your user model
...
def login(request):
    ...
    if Registrationform.objects.filter(Username=User_name).exists():
        user = Registrationform.objects.filter(USER_NAME=User_name).first()
        if user:
            if check_password(Password, user.PASSWORD):
                ...
                response.set_cookie('login_cookie', 'cookie_value', max_age=3600, httponly=True, secure=True, samesite='Strict')
...
def display_staffs(request):
    ...
    if Staffs.objects.filter(USER_NAME=Username).exists():
      data=Staffs.objects.get(USER_NAME=Username)
      
      serializer=StaffsSerializer(data)
...
def display_Patients(request):
    ...
    if Patients.objects.filter(USER_NAME=Username).exists():
      data=Patients.objects.get(USER_NAME=Username)
      
      serializer=PatientsSerilizers(data)
...
def search_doctor(request):
    ...
    if query:
        doctors = Doctors.objects.filter(Q(USER_NAME__icontains=query) | Q(PHONE_NUMBER__icontains=query))
        ...
        if doctors.exists():
            ...
            return JsonResponse({'status': 'error', 'message': 'No matching doctor found'}, status=404)
    else:
        return JsonResponse({'status': 'error', 'message': 'Query parameter missing or empty'}, status=400)
...
def search_Patients(request):
    ...
    if query:
        patients = Patients.objects.filter(Q(USER_NAME__icontains=query) | Q(PHONE_NUMBER__icontains=query))
        
        if patients.exists():
            serializer = PatientsSerilizers(patients,many=True)
            return JsonResponse({'status': 'success', 'data': serializer.data}, status=200)
        else:
            return JsonResponse({'status': 'error', 'message': 'No matching patients found'}, status=404)
    else:
        return JsonResponse({'status': 'error', 'message': 'Query parameter missing or empty'}, status=400)
@csrf_exempt
def search_Staffs(request):
    ...
    if query:
        staffs = Staffs.objects.filter(Q(USER_NAME__icontains=query) | Q(PHONE_NUMBER__icontains=query))
        
        if staffs.exists():
            serializer = StaffsSerializer(staffs,many=True)
            return JsonResponse({'status': 'success', 'data': serializer.data}, status=200)
        else:
            return JsonResponse({'status': 'error', 'message': 'No matching staff found'}, status=404)
    else:
        return JsonResponse({'status': 'error', 'message': 'Query parameter missing or empty'}, status=400)