...
def search_doctor(request):
    ...
    if query:
        data=Doctors.objects.filter(Q(USER_NAME=query) | Q(PHONE_NUMBER=query))
        serializer=DoctorsSerializer(data)
        
        return JsonResponse({'status':'data','data':serializer.data})
    else:
      return JsonResponse({'error':'data unvailable'})  
ef search_doctor(request):
