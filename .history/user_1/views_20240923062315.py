...
def booking(request):
    if request.method=='POST':
        ...
        if Booking.objects.filter(PHONE_NUMBER=Phone_number).exists():
            ...
            Booking.objects.create(NAME=Name,AGE=Age,DOB=Dob,PHONE_NUMBER=Phone_number,EMAIL=Email,TOKEN=new_token,GENTER=Genter)  
            return HttpResponse(f"your booking successfully, T number is {new_token}")
    else:
