...
def booking(request):
    if request.method == 'POST':
        ...
        if last_token:
            ...
            new_token = 1  
        Booking.objects.create(NAME=Name,AGE=Age,DOB=Dob,PHONE_NUMBER=Phone_number,EMAIL=Email,GENDER=Gender,
            token=new_token
        )

        return HttpResponse(f"Your booking was successful. Token number is {new_token}")

    else:
        return JsonResponse({'error': "Booking failed. Invalid request method."})
