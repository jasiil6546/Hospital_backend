    ...
    if request.method =='POST':
            ...
            if Registrationform.objects.filter(USER_NAME=User_name).exists(): 
                ...
                if Category == "Doctors":
                    ...
                    Registrationform.objects.create(NAME=Name,USER_NAME=User_name,EMAIL=Email,CATEGORY=Category,PHONE_NUMBER=Phone_number,AGE=Age,GENDER=Gender)
...
def data_of_Doctors(request):
   if request.method=="POST":
      ...
      if Doctors.objects.filter(USER_NAME=User_name).exists():
          data=Doctors.object.get(USER_NAME=User_name)
