...
def login(request):
 if request.method == 'POST':
    ...
    if Registrationform.objects.filter(USER_NAME=User_name,PASSWORD=Password).exists():data=Registration.objects.get(USER_NAME=User_name)
      if data.USER_NAME==User_name and check_password(Password,data.PASSWORD): 
