...
def booking(request):
    if request.method=='POST':
        ...
        if Booking.objects.filter(PHONE_NU=Phone_number).exists():
            return JsonResponse({'error':"your are aleady booked"})
        else:
           
            last_token=Booking.objects.all().order_by('Token').last()
            Token=last_token.Token+1

            Booking.objects.create(NAME=Name,AGE=Age,DOB=Dob,PHONE_NUMBER=Phone_number,EMAIL=Email,
                                  GENTER=Genter,TOKEN=Token)  
            return HttpResponse(f"your booking successfully, Token number is {Token}")
    else:
      return JsonResponse({'error':"booking is failed"})
