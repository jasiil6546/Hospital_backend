...
def delete_data(request):
      if request.method=="POST":
       User_name=request.POST.get('username')         
       if Registrationform.objects.filter(USER_NAME=User_name).exists():
         data=Registrationform.objects.get(USER_NAME=User_name).delete()
         user=data.Cat
         
         if user=="Doctors":
                    Doctors.objects.get(USER_NAME=User_name).delete()
                    Registrationform.objects.get(USER_NAME=User_name).delete()
                    return JsonResponse({'status':'deleted successfully data of doctors'})
         elif user=='Staffs':
                    Staffs.objects.get(USER_NAME=User_name).delete()  
                    Registrationform.objects.get(USER_NAME=User_name).delete()
                    return JsonResponse({'status':'deleted successfully data of staffs'})
         else:
                    Patients.objects.get(USER_NAME=User_name).delete()  
                    Registrationform.objects.get(USER_NAME=User_name).delete()
                   
                    return JsonResponse({'status':'deleted successfully data of patients'})
      else:
           return JsonResponse({'error':'user deleting failed successfully'})        