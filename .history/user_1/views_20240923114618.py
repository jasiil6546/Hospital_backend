...
def delete_data(request):
      if request.method=="POST":
       ...
       if Registrationform.objects.filter(USER_NAME=User_name).exists():
         ...
         if user=="Doctors":
                    Doctors.objects.create(USER_NAME=User_name).delete()
                    Registrationform.objects.create(USER_NAME=User_name,PASSWORD=make_password(Password),EMAIL=Email,CATEGORY=Category,PHONE_NUMBER=Phone_number,AGE=Age,GENDER=Gender)
